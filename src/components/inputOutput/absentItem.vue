<template>
  <div class="tw-w-full md:tw-max-w-[120px] tw-bg-[#EBF5FF] tw-rounded-[15px] tw-flex tw-items-center md:tw-items-start md:tw-flex-col tw-text-black tw-py-3 tw-px-4 md:tw-p-0">
    <div class="tw-h-7 md:tw-w-full tw-flex tw-items-center md:tw-px-3 md:tw-border-b tw-border-white tw-ml-3 md:tw-ml-0">
      <span class="tw-text-sm tw-font-medium">مرخصی</span>
      <v-icon
        :icon="absentStatusIcon"
        size="13"
        class="tw-cursor-pointer md:tw-ml-auto tw-mr-1"
      />
      <v-menu
        :width="130"
        location="bottom"
    ...
    <div class="tw-w-full tw-flex md:tw-flex-col md:tw-px-3 md:tw-py-1">
      <div class="tw-ml-4 md:tw-ml-0">
        <v-icon
          ...
          size="12"
          class="tw-ml-1 tw-stroke-[#0080FF]"
        />
        <span class="tw-text-sm tw-font-medium tw-flex-grow">{{ toHHmm(data.StartTime) }}</span>
      </div>
      <div>
        <v-icon
          icon="custom:arrowUp"
          ...
          class="tw-ml-1 tw-stroke-[#0080FF]"
        />
        <span class="tw-text-sm tw-font-medium tw-flex-grow">{{ toHHmm(data.EndTime) }}</span>
      </div>
    </div>
    <v-menu
      :width="130"
...
let absentStatus = computed(() => {
  return $getEnum($enums.absentStatus, 'key', propItems.data?.AbsentStatus)
})

const absentStatusIcon = computed(() => {
  return {
    inProgress: 'custom:exclamationMark',
    accepted: 'custom:checkCircle',
    rejected: 'custom:exclamationMark',
  }[absentStatus.value.title]
})
