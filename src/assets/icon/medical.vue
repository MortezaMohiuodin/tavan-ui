<template>
    <svg width="800px" height="800px" viewBox="0 0 96 96" xmlns="http://www.w3.org/2000/svg">
        <title />
        <path d="M64,8H32a8,8,0,0,0-8,8V26h7V19a4,4,0,0,1,4-4H61a4,4,0,0,1,4,4v7h7V16A8,8,0,0,0,64,8Z"
            style="fill:#575072" />
        <rect height="62" rx="8" ry="8" style="fill:#eaeaea" width="92" x="2" y="26" />
        <path d="M64,8H32a8,8,0,0,0-8,8V26h7V19a4,4,0,0,1,4-4H61a4,4,0,0,1,4,4v7h7V16A8,8,0,0,0,64,8Z"
            style="fill:none;stroke:#281a3b;stroke-linecap:round;stroke-linejoin:round;stroke-width:4px" />
        <path
            d="M56,66h9a3,3,0,0,0,3-3V53a3,3,0,0,0-3-3H56V41a3,3,0,0,0-3-3H43a3,3,0,0,0-3,3v9H31a3,3,0,0,0-3,3V63a3,3,0,0,0,3,3h9v9a3,3,0,0,0,3,3H53a3,3,0,0,0,3-3Z"
            style="fill:#f47c6d" />
        <path
            d="M56,66h9a3,3,0,0,0,3-3V53a3,3,0,0,0-3-3H56V41a3,3,0,0,0-3-3H43a3,3,0,0,0-3,3v9H31a3,3,0,0,0-3,3V63a3,3,0,0,0,3,3h9v9a3,3,0,0,0,3,3H53a3,3,0,0,0,3-3Z"
            style="fill:none;stroke:#281a3b;stroke-linecap:round;stroke-linejoin:round;stroke-width:4px" />
        <path d="M86,26H71A15,15,0,0,1,86,41V73A15,15,0,0,1,71,88H86a8,8,0,0,0,8-8V34A8,8,0,0,0,86,26Z"
            style="fill:#bacece" />
        <rect height="62" rx="8" ry="8"
            style="fill:none;stroke:#281a3b;stroke-linecap:round;stroke-linejoin:round;stroke-width:4px" width="92" x="2"
            y="26" />
    </svg>
</template>